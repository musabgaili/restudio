 ...
 * @param {Array} apiNodes - Array of node objects from API (optional)
 */
function initializeNodeManager(apiNodes = []) {
    // Initialize with empty arrays if no nodes are provided
    ...
    nodes = apiNodes.map((node) => ({
        id: node.id,
        virtual_tour_id: node.virtual_tour_id,
        name: node.name,
        panorama: node.media && node.media.length > 0 && node.media[0].original_url ? node.media[0].original_url : `/storage/${node.panorama_path}`,
        panorama_path: node.panorama_path,
        media: node.media || [],
        thumbnail: node.media && node.media.length > 0 && node.media[0].original_url ? node.media[0].original_url : `/storage/${node.thumbnail_path}`,
    ...
    // Set current node ID to the first node if available
    if (nodes.length > 0) {
...
function createNodes(images) {
    // Don't overwrite existing nodes to keep API data
    if (nodes.length > 0) {
